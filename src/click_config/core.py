...
from typing import (
    Any, Callable, Literal, Tuple, Type, Optional,
    get_origin, get_args
)
...
def config_class(cls: Optional[Type] = None, /, **kw):
    ...
    def wrap(cls):
        ...
        cls = dataclass(order=False, **kw)(cls)
        ...
        def click_options(cls, func=None, *, name="config"):
        ...
        cls.click_options = classmethod(click_options)
...
def add_click_options(func: Callable, config_cls: Type, name: str) -> Callable:
    ...
    for _field in fields(config_cls):
        ...
        func = click.option(
            *_field.param_decls, **attrs
        )(func)

    # add option for reading values from a file
    func = click.option(
        f"--{name}", default=None, type=click.Path(
            exists=True, file_okay=True, dir_okay=False, readable=True))(func)
